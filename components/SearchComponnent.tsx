  ...
  return (
    <div className="relative">
      ...
      onClick={()=>setIsclick(true)}
        ...
        } md:w-[400px] lg:w-[500px] xl:w-[573px] cursor-pointer`}
      >
        <input
            ...
            <div className="space-y-3">
              ...
              <ul className="space-y-2 text-sm text-gray-600">
                {[...Array(3)].map((_, index) => (
                  <li
                    key={index}
                    className="flex items-center text-customGray text-base font-medium space-x-4"
                  >
      ...
      {!isLoading && (
        <div
          ...
          className={`absolute ${
            isOpen && "z-[900]"
          } top-full left-0 w-full bg-white shadow-lg rounded-lg p-4 mt-3 transition-all duration-500 ease-out 
          ${
            isOpen
          ...
          <div className="space-y-4">
            {!searchValue &&
              searchQuery?.data?.message ===
                "گزینه q باید تنها شامل حروف باشد" && (
                <div>
                  ...
                  <ul className="  text-gray-600">
                    {data?.data?.search_histories.map(
                      (item: { title: string }, _: number) => (
                        <div
                          key={_}
            ...
            {searchValue.trim().split(" ").length < 2 &&
            ...
            {!isLoadingQuery &&
              ...
              searchQuery?.data?.categories?.length === 0 && (
                <div className="w-full flex justify-center text-customGray font-medium">
                  موردی برای {searchValue} یافت نشد !!!{" "}
                </div>
              )}
                ...
                <div>
                  <div className="relative">
                    ...
                    >
                      ...
                      >
                        <path
                          d="M0.339844 10.7656L10.4648 0.640625C10.6055 0.5 10.8398 0.5 10.9805 0.640625C11.1211 0.78125 11.1211 1.01562 10.9805 1.15625L1.13672 11L10.9805 20.8906C11.1211 21.0312 11.1211 21.2656 10.9805 21.4062C10.8398 21.5469 10.6055 21.5469 10.4648 21.4062L0.339844 11.2812C0.199219 11.1406 0.199219 10.9062 0.339844 10.7656Z"
                          fill="#515869"
                        />
                      </svg>
                    </div>
                    ...
                    >
                      ...
                      >
                        <path
                          d="M11.4766 10.7656C11.6172 10.9062 11.6172 11.1406 11.4766 11.2812L1.35156 21.4062C1.21094 21.5469 0.976562 21.5469 0.835938 21.4062C0.695312 21.2656 0.695312 21.0312 0.835938 20.8906L10.6797 11L0.835938 1.15625C0.695312 1.01562 0.695312 0.78125 0.835938 0.640625C0.976562 0.5 1.21094 0.5 1.35156 0.640625L11.4766 10.7656Z"
                          fill="#515869"
                        />
                      </svg>
                    </div>
            ...
            {!isLoadingQuery &&
              ...
              searchQuery?.data?.categories?.length > 0 && (
                <div>
                  {searchQuery?.data?.categories.map(
                    (item: { title: string }, _: number) => (
                      <div key={_} className="flex items-center">
                        <span className="text-customGray">
                          <SearchIcon />
                        </span>
                        <p className="text-customGray mr-4">{item.title}</p>
                      </div>
                    )
                  )}
                  <p className="text-customGray mt-2">
                    در دسته <span className="text-aquaBlue">محصول</span>
                  </p>
                </div>
              )}
            ...
            {!searchQuery?.data?.products &&
              !searchQuery?.data?.products?.length && (
                <div>
                  ...
                  <ul className="space-y-2 text-sm text-gray-600">
                    {data?.data?.popular_searches.map(
                      (item: { title: string }, _: number) => (
                        <Link
                          href={`/search/${item.title}`}
                          onClick={() => {
